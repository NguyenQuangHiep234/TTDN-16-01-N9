...
class Projects(models.Model):
    ...
    risk_ids = fields.One2many('risk.assessment', 'project_id', string='Rủi ro dự án')
    risk_count = fields.Integer('Số lượng rủi ro', compute='_compute_risk_stats', store=True)
    critical_risk_count = fields.Integer('Rủi ro nghiêm trọng', compute='_compute_risk_stats', store=True)
    high_risk_count = fields.Integer('Rủi ro cao', compute='_compute_risk_stats', store=True)
    risk_metric_ids = fields.One2many('risk.metric', 'project_id', string='Chỉ số rủi ro')
    
    @api.depends('risk_ids', 'risk_ids.risk_level', 'risk_ids.status')
    def _compute_risk_stats(self):
        """Tính toán thống kê rủi ro"""
        for project in self:
            active_risks = project.risk_ids.filtered(lambda r: r.status not in ['resolved', 'accepted'])
            project.risk_count = len(active_risks)
            project.critical_risk_count = len(active_risks.filtered(lambda r: r.risk_level == 'critical'))
            project.high_risk_count = len(active_risks.filtered(lambda r: r.risk_level == 'high'))
    
    def action_analyze_risks(self):
        """Chạy AI phân tích rủi ro cho dự án này"""
        self.ensure_one()
        ai_engine = self.env['risk.ai.engine']
        risks = ai_engine.analyze_project_risks(self.id)
        ...
        return {
            ...
            'params': {
                ...
                'message': f'Đã tự động phát hiện {len(risks)} rủi ro tiềm ẩn cho dự án {self.projects_name}',
                'type': 'success' if len(risks) == 0 else 'warning',
                'sticky': False,
            }
        }
    
    def _auto_analyze_risks_on_changes(self):
        """Tự động chạy AI khi có thay đổi quan trọng trong dự án"""
        for project in self:
            # Chỉ phân tích cho dự án đang active
            if project.status in ['in_progress', 'not_started']:
                try:
                    ai_engine = self.env['risk.ai.engine']
                    ai_engine.analyze_project_risks(project.id)
                    _logger.info(f"Auto AI analysis completed for project {project.projects_id}")
                except Exception as e:
                    _logger.error(f"Auto AI analysis failed for project {project.projects_id}: {str(e)}")
    
    @api.model
    def create(self, vals):
        """Override create để tự động phân tích rủi ro khi tạo dự án mới"""
        project = super(Projects, self).create(vals)
        # Chạy AI analysis sau khi tạo xong
        if project.status in ['in_progress', 'not_started']:
            project._auto_analyze_risks_on_changes()
        return project
    
    def write(self, vals):
        """Override write để tự động phân tích lại khi có thay đổi quan trọng"""
        result = super(Projects, self).write(vals)
        
        # Các field trigger AI re-analysis
        important_fields = ['progress', 'status', 'actual_end_date', 'start_date']
        
        # Kiểm tra xem có field quan trọng nào thay đổi không
        if any(field in vals for field in important_fields):
            self._auto_analyze_risks_on_changes()
